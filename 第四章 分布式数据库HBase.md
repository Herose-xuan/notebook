...
通过官网下载地址（✅**官网下载地址**：[HBase下载](https://hbase.apache.org/downloads.html)），下载hbase 2.3.5的安装包到本地指定目录。运行下面的命令，解压安装包至`/opt`目录下：
...
使用 `put` 命令插入课程数据。每门课对应一行，属性分别是课程号（C_No）、课程名（C_Name）和学分（C_Credit）
